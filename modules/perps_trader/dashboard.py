"""Declarative dashboard for the Perps Trader module."""
...
def _dir_badge(is_long) -> Badge:
    return Badge("LONG", "success") if is_long else Badge("SHORT", "danger")


def _decision_badge(decision: str) -> Badge:
    m = {"OPEN_LONG": "success", "OPEN_SHORT": "danger", "CLOSE": "warning", "HOLD": "default", "APPROVE_USDC": "warning"}
...
def _status_badge(status: str) -> Badge:
    ...
    fail = {"failed", "reverted", "broadcast_failed", "tx_failed"}
    if status in ok:
...
class PerpsTraderDashboard(Dashboard):
    title = "Perps Trader"
    ...
    def layout(self) -> Layout:
        ...
        usdc_approved = status.get("usdc_approved") == "true"
        worker_label = "Running" if worker_running else "Stopped"

        # Stats
        stat_list = [
            Stat("Positions", stats.get("open_positions", 0)),
            Stat("Collateral", f"${stats.get('total_collateral_deployed', 0)}"),
        ...
        # Meta — status line
        usdc_label = "Approved" if usdc_approved else "Not Approved"
        meta = {
            "Worker": worker_label,
            "USDC": usdc_label,
        }
        ...
        # Warnings
        warnings = []
        if not usdc_approved:
            warnings.append("USDC not approved — use Approve USDC button before trading")

        # Buttons
        button_list = [
            Button("Refresh Prices", "/rpc/refresh"),
            Button("Sync Positions", "/rpc/sync_positions"),
        ...
        for r in positions:
            ...
            pos_rows.append([
                r.get("pair_name", ""),
                _dir_badge(r.get("is_long")),
                f"${float(r.get('collateral_usdc', 0)):.2f}",
                f"{float(r.get('leverage', 0)):.1f}x",
                _price_fmt(r.get("open_price")),
                _price_fmt(r.get("current_price")),
        ...
        for r in trades:
            ...
            trade_rows.append([
                r.get("pair_name", ""),
                _dir_badge(r.get("is_long")),
                f"${float(r.get('collateral_usdc', 0)):.2f}" if r.get("collateral_usdc") is not None else "\u2014",
                f"{float(r.get('leverage', 0)):.1f}x" if r.get("leverage") is not None else "\u2014",
                _price_fmt(r.get("open_price")),
        ...
        return Layout(
            ...
            tables=[
                Table(
                    columns=["Pair", "Direction", "Collateral", "Leverage", "Entry", "Current", "Liq. Price", "P&L %", "Unrealized"],
                    rows=pos_rows,
                    title="Open Positions",
                    empty="No open positions.",
                ),
                Table(
                    columns=["Pair", "Direction", "Collateral", "Leverage", "Entry", "Exit", "Realized P&L", "P&L %", "Closed"],
                    rows=trade_rows,
                    title="Trade History",
                    empty="No trades yet.",
                ),
                Table(
                    columns=["ID", "Decision", "Pair", "Reason", "Status", "Time"],
                    rows=dec_rows,
